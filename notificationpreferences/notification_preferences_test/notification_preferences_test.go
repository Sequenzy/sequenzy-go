...
package notification_preferences_test
...
func TestNotificationPreferencesGetWithWireMock(
	t *testing.T,
) {
	...
	VerifyRequestCount(t, "TestNotificationPreferencesGetWithWireMock", "GET", "/notification-preferences", nil, 1)
}

func TestNotificationPreferencesUpdateWithWireMock(
	t *testing.T,
) {
	WireMockBaseURL := os.Getenv("WIREMOCK_URL")
	if WireMockBaseURL == "" {
	...
	_, invocationErr := client.NotificationPreferences.Update(
		context.TODO(),
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestNotificationPreferencesUpdateWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestNotificationPreferencesUpdateWithWireMock", "PATCH", "/notification-preferences", nil, 1)
}
