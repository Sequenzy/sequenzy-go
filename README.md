...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Sequenzy%2FGo)

The Sequenzy Go library provides convenient access to the Sequenzy APIs from Go.
...
```go
response, err := client.AbTests.AddVariant(...)
if err != nil {
...
```go
response, err := client.AbTests.WithRawResponse.AddVariant(...)
if err != nil {
    return err
}
...
response, err := client.AbTests.AddVariant(ctx, ...)
```
...
response, err := client.AbTests.AddVariant(ctx, request, ...)
```
